...
@dataclass
class DPORegConfig(transformers.TrainingArguments):

    beta1: Optional[float] = field(
        default=0.1,
        metadata={"help": "The beta factor in DPORag loss. Higher beta means less divergence from the initial policy."},
    )
    ...
    loss_type: Optional[str] = field(
        default="DPO_Reg",
        metadata={"help": ("loss type for training")},
    )
    logging_first_step: bool = field(
        default=True,
        metadata={"help": ("Whether to log and evaluate the first global_step or not.")},
