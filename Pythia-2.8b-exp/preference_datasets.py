...
def get_tldr(split: str, silent: bool = False, cache_dir: str = None, sft_mode: bool = False) -> Dict[str, Dict[str, Union[List[Tuple[int, int]], List[str], str]]]:
    """Load the Anthropic Helpful-Harmless dataset from Huggingface and convert it to the necessary format.
       ...
       For this dataset, the sft_target is just the chosen response.
    """
    print(f'Loading TLDR dataset ({split} split) from Huggingface...')

    if sft_mode:
        with open('datasets/tldr_filtered/{}.json'.format(split), 'r') as f:
            dataset = json.load(f)
    else:
        with open('datasets/tldr/{}.json'.format(split), 'r') as f:
            dataset = json.load(f)
    print('done')
    ...
    data = defaultdict(lambda: defaultdict(list))
    for index, row in enumerate(tqdm.tqdm(dataset, desc='Processing TLDR', disable=silent)):
        prompt, chosen, rejected = split_prompt_and_responses(row, sft_mode)
        prompt = str(index)+prompt
        responses = [chosen, rejected]
        n_responses = len(data[prompt]['responses'])
        data[prompt]['pairs'].append((n_responses, n_responses + 1))
        data[prompt]['responses'].extend(responses)
...
def get_dataset(name: str, split: str, silent: bool = False, cache_dir: str = None, sft_mode: bool = False):
    ...
    if name == 'shp':
        ...
        data = get_tldr(split, silent=silent, cache_dir=cache_dir, sft_mode=sft_mode)
    else:
...
def get_batch_iterator(names: List[str],
    ...
    with TemporarilySeededRandom(seed):
        ...
        for name in names:
            ...
            for index, (prompt, data) in enumerate(get_dataset(name, split, silent=silent, cache_dir=cache_dir, sft_mode=sft_mode).items()):
                if name == 'tldr':
                    flat_data.append((prompt[len(str(index)):], data['responses'], data['pairs'], data['sft_target'], truncation_mode))
                else:
