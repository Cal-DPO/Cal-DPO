...
def preference_loss(policy_chosen_logps: torch.FloatTensor,
    ...
    if shift:
        regularization_losses = F.mse_loss(chosen_rewards, torch.tensor((1-shift)/beta).expand_as(chosen_rewards).to(chosen_rewards.device), reduction='none') + F.mse_loss(rejected_rewards, torch.tensor((0-shift)/beta).expand_as(rejected_rewards).to(rejected_rewards.device), reduction='none')
    else:
        regularization_losses = torch.tensor(0.0).expand_as(chosen_rewards).to(chosen_rewards.device)
    
    losses = dpo_losses + regularization_losses

    return losses, dpo_losses, regularization_losses, beta * chosen_rewards.detach(), beta * rejected_rewards.detach()
...
class BasicTrainer(object):
    def __init__(self, policy: nn.Module, config: DictConfig, seed: int, run_dir: str, reference_model: Optional[nn.Module] = None, rank: int = 0, world_size: int = 1):
        ...
        print('config.n_examples',config.n_examples)
    ...
    def get_batch_samples(self, batch: Dict[str, torch.LongTensor]) -> Tuple[str, str]:
        ...
        if self.config.loss.name in {'dpo', 'ipo', 'cal_dpo'}:
    ...
    def get_batch_metrics(self, batch: Dict[str, Union[List, torch.LongTensor]], loss_config: DictConfig, train=True):
        ...
        if loss_config.name in {'dpo', 'ipo'}:
            ...
            losses, dpo_losses, regularization_losses, chosen_rewards, rejected_rewards = preference_loss(
            ...
            all_devices_dpo_losses = all_gather_if_needed(dpo_losses.detach(), self.rank, self.world_size)
            all_devices_regularization_losses = all_gather_if_needed(regularization_losses.detach(), self.rank, self.world_size)
            metrics[f'dpo_loss/{train_test}'] = all_devices_dpo_losses.cpu().numpy().tolist()
            metrics[f'reg_loss/{train_test}'] = all_devices_regularization_losses.cpu().numpy().tolist()


        elif loss_config.name == 'cal_dpo':

            policy_chosen_logps, policy_rejected_logps = self.concatenated_forward(self.policy, batch)
            with torch.no_grad():
                reference_chosen_logps, reference_rejected_logps = self.concatenated_forward(self.reference_model, batch)

            loss_kwargs = {'beta': loss_config.beta, 'shift':loss_config.shift, 'reference_free': loss_config.reference_free, 'label_smoothing': loss_config.label_smoothing, 'ipo': False}
...
class FSDPTrainer(BasicTrainer):
    def __init__(self, policy: nn.Module, config: DictConfig, seed: int, run_dir: str, reference_model: Optional[nn.Module] = None, rank: int = 0, world_size: int = 1):
        ...
        if config.loss.name in {'dpo', 'ipo', 'cal_dpo'}:
